      ...
      <div ref={ref} style={{ height: "100vh", overflowY: "auto", scrollSnapType: "y mandatory", position: "relative", zIndex: 1 }}>
        ...
        <div style={S}>
          <div style={{ maxWidth: 820, textAlign: "center" }}>
            ...
            <h2 style={{ fontSize: "clamp(26px, 4.5vw, 46px)", fontWeight: 700, fontFamily: "'Outfit', sans-serif", margin: 0, lineHeight: 1.2, opacity: active === 1 ? 1 : 0, transform: active === 1 ? "translateY(0)" : "translateY(25px)", transition: "all 0.6s ease" }}>
              4G gave us <span style={{ color: P.gold }}>fast pipes</span>.<br />5G SA gives us <span style={{ color: P.cyan }}>programmable networks</span>.
            </h2>
            <p style={{ fontSize: 16, color: P.dim, marginTop: 20, lineHeight: 1.7, opacity: active === 1 ? 1 : 0, transition: "opacity 0.6s ease 0.3s" }}>
              4G LTE was designed as a single highway — everyone shares the same road, same rules, same speed limits.
              5G Standalone replaces this with a programmable infrastructure where every application gets its own purpose-built network with guaranteed performance.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginTop: 36 }}>
              {[
                { icon: "🧠", title: "Cloud-Native Core", desc: "Microservices-based architecture that scales dynamically — not the monolithic 4G core" },
                { icon: "🔀", title: "Network Slicing", desc: "Multiple isolated virtual networks on one infrastructure, each with custom SLAs" },
                { icon: "⚡", title: "Ultra-Low Latency", desc: "Sub-millisecond latency with dedicated 5G control plane — no 4G fallback" },
              ].map((item, i) => (
                <div key={i} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 16, padding: "24px 18px", opacity: active === 1 ? 1 : 0, transform: active === 1 ? "translateY(0)" : "translateY(20px)", transition: `all 0.5s ease ${0.3 + i * 0.1}s` }}>
                  <div style={{ fontSize: 28, marginBottom: 12 }}>{item.icon}</div>
                  <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8 }}>{item.title}</div>
                  <div style={{ fontSize: 13, color: P.dim, lineHeight: 1.55 }}>{item.desc}</div>
                </div>
              ))}
            </div>
          </div>
